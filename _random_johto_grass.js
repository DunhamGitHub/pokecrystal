...
$('#process').click(function() {
  ...
  const newTargetText = text.replace(/db\s\d+,\s(.*?)(?=\n)/g, (match, $1) => {
    // Return the replacement leveraging the parameters.
    		
        //console.log('match', match);
        //console.log('dol 1', $1);
        var add = match.substring(0, match.indexOf($1));
        //console.log('add', add);
      return add+species[Math.floor(Math.random() * species.length)];

  });

  console.log('newTargetText', newTargetText);

  $('#out').val(newTargetText);
});
